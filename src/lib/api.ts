...
export async function getListings(filters: ListingFilters = {}): Promise<Listing[]> {
  // only fetch listings available within last 24h or in the future
  const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

  let q = supabase.from("listings").select(LISTING_FIELDS)
    .gte("available_from", cutoff)
    .order("available_from", { ascending: true });

  if (filters.destination?.trim()) q = q.ilike("to_city", `%${filters.destination.trim()}%`);
  if (typeof filters.minGel === "number" && isFinite(filters.minGel)) q = q.gte("price_gel", filters.minGel);
  if (typeof filters.maxGel === "number" && isFinite(filters.maxGel)) q = q.lte("price_gel", filters.maxGel);
  if (filters.onlyWithSpots) q = q.gt("spots_available", 0);
...
export async function getListingById(id: string): Promise<Listing | null> {
  const { data, error } = await supabase
    .from("listings").select(LISTING_FIELDS).eq("id", id).maybeSingle();
